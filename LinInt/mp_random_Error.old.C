...
int main()
{
  ...
  mp_real diffdataL2_mp   = mp_real(0.0);
  mp_real diffrandomL2_mp = mp_real(0.0);
  double diffdataL2   = 0.0;
  ...
  for(int i = 0; i <= m; i++)
    {
      x = x1 + i * h2;
      Random_mp = mpf1(x);      
      diffdataL2_mp += (Random_mp * Random_mp);
      Random    = dble(Random_mp);
      xd        = dble(x);
      dout << xd << '\t' << Random << endl; //Ausgabe Rauschen
      
    }
  diffdataL2_mp *= h2;

  cout <<"*** End: L2-Norm Error Data ***" << endl;
  cout << "*** L2-Datenfehler = " << dble(diffdataL2_mp) << " ***" << endl;
  cout << '\v';
  ...
  for(int i = 0; i<= m; i++){
    ...
    diffrandomL2_mp += h2 * RandomReg_mp * RandomReg_mp ;
    ...
    cout << x << Random_mp << RandomReg_mp << endl;
