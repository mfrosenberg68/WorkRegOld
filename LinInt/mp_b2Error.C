...
int main()
{
  ...
  ofstream out1("Daten_b2ErrorGauss.out");
  out1 << setiosflags(ios::uppercase);
  
  ofstream out("LIP_b2Error.out");
  out << setiosflags(ios::uppercase);

  ofstream outs("Y2D_b2Error.out");
  outs  << setiosflags(ios::uppercase);
  
  ofstream rout("REGLIP_b2ErrorGauss.out");
  rout  << setiosflags(ios::uppercase);

  ofstream lout("L2-Normb2Gauss.out");
  ...
  double asymp; //asymp. Abschaetzung
  double gamma = 0.5/b1;
  
  //  asymp = pow(2.0*dpi,1.5) * epsx0/gamma * exp(1.125/(gamma*gamma));
  asymp = pow(2.0*dpi,1.5) * epsx0/gamma * exp(0.25/(gamma*gamma));
  //  asymp = (pow(2.0*dpi,1.5)/gamma) * exp(1.125/(gamma*gamma));


  cout << "*** L2-Datenfehler = " << epsx0 << " ***" << endl;
  cout << "*** L2-Norm Error  = " << Error << " ***" << endl; 
  cout << "*** asymp. Error   = " << asymp << " ***" << endl; 

  lout << "b " << '\t' << "L2-Datenfehler" << '\t' << " L2-Error" << '\t' << "asymp L2-Error " << endl;
  lout << b1 << '\t' << epsx0 << '\t' << Error << '\t' << asymp << endl;
