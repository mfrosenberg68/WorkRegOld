...
int main()
{
  ...
  ofstream out("TP-CD-DATEN-FFT-REAL.out");
  ofstream ugout("TP-CD-DATEN-FFT-IMA.out");
  ofstream rfout("CD-FFT2REG.out");

  String data = "TP-DATEN-CD-beta";
  data = data + strdup(fix3(beta)) + "-xb" + xb +"-N" + n +".out";
  ofstream fout(data());

  String regout = "TP-REGSOLUTION-CD-beta";
 ...
 /* Umskalierung des Ergebnisses */
  ...
  cout << "*** kmax = " <<.5*(n+1)*dk <<'\t'<< F(.5*(n+1)*dk,beta)  << '\t' << daten[n] << '\t' << RelError(F(.5*(n+1)*dk,beta),daten[n])<<" ***" << endl; 
...
/* nur positive Frequenzen ausgeben */
  ...
  out << 0 << '\t' << 0 << '\t' << F(0,beta) << '\t' << daten[0] << '\t' << fabs(daten[0]) << '\t' << RelError(F(0,beta),fabs(daten[0])) << endl; 

  out << 1 << '\t' << .5*(n+1)*dk << '\t' << F(.5*(n+1)*dk,beta) << '\t' << daten[1] << '\t' << fabs(daten[1]) << '\t' << RelError(F(.5*(n+1)*dk,beta),fabs(daten[1])) << endl; 
