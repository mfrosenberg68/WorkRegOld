...
int main()
{
  ...
  double* const fftdaten = new double [m+1]; //Datenarray, " C++ build-in type"
 ...
 /* Umskalierung des Ergebnisses */
  ...
  for (unsigned long i = 0; i < m+1; i++) fftdaten[i] = daten[i] *dx; 
  ...
  for(unsigned long i = 0; i < (n-1)/2; i++)
    ...
    regdaten[j] = StabFkt(k,b)*Kern2(k)*fftdaten[j]; //Realteil der reg. FFT
    regdaten[jj] = StabFkt(k,b)*Kern2(k)*fftdaten[jj]; //Imaginaerzeil der reg. FFT

    fft_daten = fftdaten[j];
    Error_rel = RelError(F(k,beta1),fabs(fftdaten[j]));
    
    out << j << '\t' << k << '\t' << F(k,beta1) << '\t' << fabs(fftdaten[j]) << '\t' << fft_daten << '\t' << Error_rel << endl; 
    
    rfout << j << '\t' << k << '\t' << regdaten[j] << '\t' << StabFkt(k,b)*Kern2(k) << '\t' << fftdaten[j] <<  endl;
    
    ugout << jj << '\t' << k << '\t' << fftdaten[jj] << '\t' << fabs(fftdaten[jj]) << endl;
    }
