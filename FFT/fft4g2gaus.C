...
int main()
{
  ...
  ofstream out("REG-GAUSS-DATEN-FFT-REAL.out");
  ofstream ugout("REG-GAUSS-DATEN-FFT-IMA.out");
  ofstream rfout("GAUSS-FFT2REG.out");

  String data = "DATEN-GAUSS-beta1";
  data = data + strdup(fix3(beta1))+"-"+strdup(fix3(t1)) + "-beta2" + strdup(fix3(beta2))+"-"+strdup(fix3(t2)) + "-a" + strdup(fix3(a)) + "-xb" + xb +"-N" + n +".out";
  ofstream fout(data());

  String regout = "REGSOLUTION-GAUSS-beta1";
  regout = regout +strdup(fix3(beta1))+"-"+strdup(fix3(t1)) + "-beta2" + strdup(fix3(beta2))+"-"+strdup(fix3(t2)) + "-a" + strdup(fix3(a)) + "-xb" + xb +"-N" + n  + "-b" +  strdup(sci3(b)) + ".out"; 
  ofstream rout(regout());
  
  String l2 = "L2-ERROR-GAUSS-beta1";
  l2 = l2 + strdup(fix3(beta1))+"-"+strdup(fix3(t1)) + "-beta2" + strdup(fix3(beta2)) +"-"+strdup(fix3(t2)) + "-a" + strdup(fix3(a)) + "-xb" + xb +"-N" + n + "-b" +  strdup(sci3(b)) + ".out";
  ofstream lout(l2());
 ...
 /* Umskalierung des Ergebnisses */
  ...
  for (unsigned long i = 0; i < m+1; i++) daten[i] *= dx; 
  ...
  for(unsigned long i = 0; i < (n-1)/2; i++)
    ...
    regdaten[j] = StabFkt(k,b)*Kern2(k)*daten[j]; //Realteil der reg. FFT
    regdaten[jj] = StabFkt(k,b)*Kern2(k)*daten[jj]; //Imaginaerzeil der reg. FFT
    ...
    rfout << j << '\t' << k << '\t' << regdaten[j] << '\t' << StabFkt(k,b)*Kern2(k) << '\t' << daten[j] <<  endl;
