...
int main()
{
  ...
  ofstream out("FGOP-DATEN-FFT-REAL.out");
  ofstream ugout("FGOP-DATEN-FFT-IMA.out");
  ofstream rfout("FGOP-FFT2REG.out");

  String data = "DATEN-FGOP-beta";
  data = data + strdup(fix3(beta)) + "-xb" + xb +"-N" + n +".out";
  ofstream fout(data());
  
  String lscan = "SCAN-BN";
  lscan = lscan + BN + "-b1" +strdup(sci3(b1)) + "-b2" + strdup(sci3(b2)) + data; 
