...
int main()
{
  ...
  cout << " *** Model: Cole-Davidson density *** " << endl;
  ...
  ofstream out("CD-DATEN-FFT-REAL.out");
  ofstream ugout("CD-DATEN-FFT-IMA.out");
  ofstream rfout("CD-FFT2REG.out");

  String data = "DATEN-CD-beta";
  data = data + strdup(fix3(beta)) + "-xb" + xb +"-N" + n +".out";
  ofstream fout(data());

  String regout = "REGSOLUTION-CD-beta";
  regout = regout + strdup(fix3(beta)) + "-xb" + xb +"-N" + n  + "-b" + strdup(sci3(b)) + ".out"; 
  ofstream rout(regout());
  
  String l2 = "TP-L2-ERROR-CD-beta";
  l2 = l2 + strdup(fix3(beta)) + "-xb" + xb +"-N" + n + "-b" + strdup(sci3(b)) + ".out";
  ofstream lout(l2());
 ...
 /* Umskalierung des Ergebnisses */
  ...
  for(int i = 0; i < (m+1)/2; i++)
    { 
      ...
      RegTh = ColeDavidson(-x0,beta,tau0); // theor. Regularisierte
